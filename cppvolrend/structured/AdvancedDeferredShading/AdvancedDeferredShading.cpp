#include "../../defines.h"
#include "AdvancedDeferredShading.h"
...
void AdvancedDeferredShading::ComputeBlocksFromVolume(
...
AdvancedDeferredShading::AdvancedDeferredShading()
...
const char* AdvancedDeferredShading::GetAbbreviationName()
{
    return "iso";
}


vis::GRID_VOLUME_DATA_TYPE AdvancedDeferredShading::GetDataTypeSupport()
{
    return vis::GRID_VOLUME_DATA_TYPE::STRUCTURED;
}

void AdvancedDeferredShading::CreateGBuffer(int width, int height)
{
    // 创建G-Buffer帧缓冲
    ...
    // 创建曲率纹理
    glGenTextures(1, &gCurvatureMax);
    glBindTexture(GL_TEXTURE_2D, gCurvatureMax);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, width, height, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, GL_TEXTURE_2D, gCurvatureMax, 0);


    glGenTextures(1, &gCurvatureMin);
    glBindTexture(GL_TEXTURE_2D, gCurvatureMin);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, width, height, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, gCurvatureMin, 0);

    glGenTextures(1, &gCurvatureDir);
    glBindTexture(GL_TEXTURE_2D, gCurvatureDir);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, width, height, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, GL_TEXTURE_2D, gCurvatureDir, 0);

    //debug
    glGenTextures(1, &gCurvatureDebug);
    glBindTexture(GL_TEXTURE_2D, gCurvatureDebug);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, width, height, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT5, GL_TEXTURE_2D, gCurvatureDebug, 0);
...
void AdvancedDeferredShading::InitCurvaturePass() {
    cp_curvature_pass = new gl::ComputeShader();
    cp_curvature_pass->AddShaderFile(CPPVOLREND_DIR"structured/AdvancedDeferredShading/curvature_pass.comp");
    if (!cp_curvature_pass->LoadAndLink()) {
        std::cerr << "Failed to load curvature pass shader" << std::endl;
        return;
    }
}    
void AdvancedDeferredShading::CreateTransferFunctions() {
    ...
    std::vector<glm::vec4> colorMap(colorMapSize);

    // 生成彩虹色映射
    for (int i = 0; i < colorMapSize; i++) {
        float t = float(i) / float(colorMapSize - 1);
        // HSV到RGB转换，创建彩虹色谱
        float h = t * 6.0f; // 色相
//        s = 0.6f;     // 饱和度
//        v = 0.2f;     // 亮度
        ...
        glm::vec3 color;
        if (h < 1.0f)      color = glm::vec3(c, x, 0.0f);
        else if (h < 2.0f) color = glm::vec3(x, c, 0.0f);
        else if (h < 3.0f) color = glm::vec3(0.0f, c, x);
        else if (h < 4.0f) color = glm::vec3(0.0f, x, c);
        else              color = glm::vec3(x, 0.0f, c);

        colorMap[i] = glm::vec4(color + m, 1.0f);
    }

    glGenTextures(1, &curvatureColorMap);
    glBindTexture(GL_TEXTURE_1D, curvatureColorMap);
    glTexImage1D(GL_TEXTURE_1D, 0, GL_RGBA32F, colorMapSize, 0, GL_RGBA, GL_FLOAT, colorMap.data());
    glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_1D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);

    // 2. 创建脊线/谷线检测纹理
    const int ridgeMapSize = 256;
    std::vector<glm::vec4> ridgeMap(ridgeMapSize * ridgeMapSize);

    for (int y = 0; y < ridgeMapSize; y++) {
        for (int x = 0; x < ridgeMapSize; x++) {
            float k1 = (float(x) / float(ridgeMapSize - 1)) * 2.0f - 1.0f;
            float k2 = (float(y) / float(ridgeMapSize - 1)) * 2.0f - 1.0f;

            // Ridge detection (positive maximum curvature)
            float ridge = std::max(0.0f, k1) * (k1 > k2 ? 1.0f : 0.0f);
            // Valley detection (negative minimum curvature)
            float valley = std::abs(std::min(0.0f, k2)) * (k2 < k1 ? 1.0f : 0.0f);

            // Ridge color (red), Valley color (blue), Alpha for blending
            ridgeMap[y * ridgeMapSize + x] = glm::vec4(ridge, 0.0f, valley,
                std::max(ridge, valley) > 0.1f ? 0.8f : 0.0f);
           // std::cout<<"x="<<x<<"y="<<y<<"ridgeMap=" << ridgeMap[y][x] << std::endl;

        }
    }

    glGenTextures(1, &ridgeValleyMap);
    glBindTexture(GL_TEXTURE_2D, ridgeValleyMap);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA32F, ridgeMapSize, ridgeMapSize, 0, GL_RGBA, GL_FLOAT, ridgeMap.data());
    ...
    for (int y = 0; y < noiseSize; y++) {
        for (int x = 0; x < noiseSize; x++) {
            // 这里用简单的随机数代替，实际应该使用更好的噪声函数
            noise[y * noiseSize + x] = float(rand()) / float(RAND_MAX);
        }
    }

    glGenTextures(1, &noiseTexture);
    glBindTexture(GL_TEXTURE_2D, noiseTexture);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, noiseSize, noiseSize, 0, GL_RED, GL_FLOAT, noise.data());
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
...
void AdvancedDeferredShading::LightingPass(vis::Camera* camera) {
    ...
    glBindTexture(GL_TEXTURE_2D, gCurvatureMax);
    cp_lighting_pass->SetUniform("gCurvatureMax", 2);

    glActiveTexture(GL_TEXTURE3);
    glBindTexture(GL_TEXTURE_2D, gCurvatureMin);
    cp_lighting_pass->SetUniform("gCurvatureMin", 3);

    glActiveTexture(GL_TEXTURE4);
    glBindTexture(GL_TEXTURE_2D, gCurvatureDir);
    cp_lighting_pass->SetUniform("gCurvatureDir", 4);

    // 绑定传输函数纹理
    glActiveTexture(GL_TEXTURE5);
    glBindTexture(GL_TEXTURE_1D, curvatureColorMap);
    cp_lighting_pass->SetUniform("curvatureColorMap", 5);

    glActiveTexture(GL_TEXTURE6);
    glBindTexture(GL_TEXTURE_2D, ridgeValleyMap);
    cp_lighting_pass->SetUniform("ridgeValleyMap", 6);
    /*
    glActiveTexture(GL_TEXTURE7);
    glBindTexture(GL_TEXTURE_2D, noiseTexture);
    cp_post_process->SetUniform("noiseTexture", 7);
    */
    // 绑定输出图像
    glBindImageTexture(7, m_lighting_texture, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RGBA32F);
    // 更新着色参数
    ...
    cp_lighting_pass->SetUniform("showValleys", m_show_valleys);
    cp_lighting_pass->SetUniform("accessibilityStrength", m_accessibility_strength);
    //cp_lighting_pass->SetUniform("screenSize", glm::vec2(width, height));
...
void AdvancedDeferredShading::Clean() {
    // 清理着色器
    ...
    glDeleteTextures(1, &gCurvatureMax);
    glDeleteTextures(1, &gCurvatureMin);
    glDeleteTextures(1, &gCurvatureDir);
    glDeleteTextures(1, &curvatureColorMap);
    glDeleteTextures(1, &ridgeValleyMap);
    glDeleteTextures(1, &noiseTexture);
   // delete cp_curvature_pass;


    gl::ExitOnGLError("AdvancedDeferredShading: Could not clean up resources");
}
...
void AdvancedDeferredShading::CurvaturePass() {
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_2D, gPosition);
    cp_curvature_pass->SetUniform("gPosition", 0);

    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_2D, gNormal);
    cp_curvature_pass->SetUniform("gNormal", 1);

    cp_curvature_pass->SetUniform("ProjectionMatrix", mycamera->Projection());

    // 设置输出
    glBindImageTexture(2, gCurvatureMax, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);
    glBindImageTexture(3, gCurvatureMin, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_R32F);
    glBindImageTexture(4, gCurvatureDir, 0, GL_FALSE, 0, GL_WRITE_ONLY, GL_RG32F);

    // 调度计算着色器
    cp_curvature_pass->Dispatch();
    glMemoryBarrier(GL_SHADER_IMAGE_ACCESS_BARRIER_BIT);

    cp_curvature_pass->Unbind();
}
bool AdvancedDeferredShading::Init(int swidth, int sheight) {
    if (IsBuilt()) Clean();
    ...
    cp_geometry_pass->AddShaderFile(CPPVOLREND_DIR"structured/AdvancedDeferredShading/geometry_pass.comp");
    ...
    cp_lighting_pass->AddShaderFile(CPPVOLREND_DIR"structured/AdvancedDeferredShading/lighting_pass.comp");
    ...
    cp_post_process->AddShaderFile(gl::PipelineShader::TYPE::VERTEX, CPPVOLREND_DIR"structured/AdvancedDeferredShading/post_process.vert");

    cp_post_process->AddShaderFile(gl::PipelineShader::TYPE::FRAGMENT, CPPVOLREND_DIR"structured/AdvancedDeferredShading/post_process.frag");
    ...
    glBindTexture(GL_TEXTURE_2D, m_lighting_texture);
    std::vector<float> clearColor(swidth * sheight * 4, 1.0f);  // 初始化为白色
...
void AdvancedDeferredShading::ReloadShaders()
{
...
bool AdvancedDeferredShading::Update(vis::Camera* camera)
{
    ...
    gl::ExitOnGLError("AdvancedDeferredShading: After Update.");
    SetOutdated();
    return true;
}
...
void AdvancedDeferredShading::FillParameterSpace(ParameterSpace& pspace)
{
...
void AdvancedDeferredShading::SetImGuiComponents()
{
    ImGui::Separator();
    ...
    ImGui::Checkbox("Show Ridges", &m_show_ridges);
    ImGui::Checkbox("Show Valleys", &m_show_valleys);
    ImGui::SliderFloat("Flow Speed", &m_flow_speed, 0.0f, 5.0f);
    ImGui::SliderFloat("Flow Scale", &m_flow_scale, 1.0f, 50.0f);
    ImGui::SliderFloat("Accessibility Strength", &m_accessibility_strength, 0.0f, 1.0f);
    ImGui::SliderFloat("saturation ", &s, 0.0f, 1.0f);
    ImGui::SliderFloat("value", &v, 0.0f, 1.0f);
    ImGui::Text("saturation: ");
    if (ImGui::DragFloat("###DeferredShadingUIIntegrationStepSizeRange", &s, 0.01f, 0.05f, 1.0f, "%.2f"))
    {
        s = std::max(std::min(s, 0.99f), 0.01f);
        SetOutdated();
    }

    if (ImGui::Button("Reset Transfer Functions")) {
        CreateTransferFunctions();
    }
}
