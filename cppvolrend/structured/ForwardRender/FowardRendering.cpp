...
void FowardRendering::ComputeBlocksFromVolume(
...
FowardRendering::FowardRendering()
...
const char* FowardRendering::GetName()
{
    return "Forward Bulin Phong Shading";
}


const char* FowardRendering::GetAbbreviationName()
{
    return "iso";
}


vis::GRID_VOLUME_DATA_TYPE FowardRendering::GetDataTypeSupport()
{
    return vis::GRID_VOLUME_DATA_TYPE::STRUCTURED;
}


void FowardRendering::Clean()
{
...
bool FowardRendering::Init(int swidth, int sheight)
{
    if (IsBuilt()) Clean();
    ...
    cp_shader_rendering->AddShaderFile(CPPVOLREND_DIR"structured/ForwardRender/ForwardRendering.comp");
    ...
    gl::ExitOnGLError("FowardRendering: Error on Preparing Models and Shaders");
...
void FowardRendering::ReloadShaders()
{
...
bool FowardRendering::Update(vis::Camera* camera)
{
    cp_shader_rendering->Bind();
    ...
    gl::Shader::Unbind();
    gl::ExitOnGLError("FowardRendering: After Update.");
    return true;
}


void FowardRendering::Redraw()
{
...
void FowardRendering::FillParameterSpace(ParameterSpace& pspace)
{
...
void FowardRendering::SetImGuiComponents()
{
    ImGui::Separator();

    ImGui::Text("Isovalue: ");
    if (ImGui::DragFloat("###FowardRendering", &m_u_isovalue, 0.01f, 0.01f, 100.0f, "%.2f"))
    {
    ...
    if (ImGui::DragFloat("###FowardRenderingUIIntegrationStepSizeSmall", &m_u_step_size_small, 0.005f, 0.01f, 1.0f, "%.2f"))
    {
    ...
    if (ImGui::DragFloat("###FowardRenderingUIIntegrationStepSizeLarge", &m_u_step_size_large, 0.01f, 0.05f, 5.0f, "%.2f"))
    {
    ...
    if (ImGui::DragFloat("###FowardRenderingtUIIntegrationStepSizeRange", &m_u_step_size_range, 0.01f, 0.05f, 0.5f, "%.2f"))
    {
